...
if [ "${1:-}" = "stop" ]; then
  ...
  podman rm -f kafka-dev sr-dev connect-dev 2>/dev/null || true
  echo "Done."
  exit 0
fi
...
podman run -d --name kafka-dev -p 9092:9092 docker.io/apache/kafka:3.9.0 >/dev/null
for i in $(seq 1 30); do
  podman exec kafka-dev $KB/kafka-broker-api-versions.sh --bootstrap-server $BS &>/dev/null && break
  sleep 2
done
echo "      Kafka ready"
...
podman run -d --name sr-dev --network host \
...
podman run -d --name connect-dev --network host \
...
for t in orders users payments notifications audit-log events; do
  p=3; [ "$t" = "events" ] && p=6
  podman exec kafka-dev $KB/kafka-topics.sh --bootstrap-server $BS \
...
for i in $(seq 1 20); do echo "{\"orderId\":$i,\"product\":\"item-$((i%5+1))\",\"amount\":$((RANDOM%1000))}"; done \
  | podman exec -i kafka-dev $KB/kafka-console-producer.sh --bootstrap-server $BS --topic orders
for i in $(seq 1 10); do echo "{\"userId\":$i,\"name\":\"user$i\",\"email\":\"u${i}@ex.com\"}"; done \
  | podman exec -i kafka-dev $KB/kafka-console-producer.sh --bootstrap-server $BS --topic users
for i in $(seq 1 15); do echo "{\"paymentId\":$i,\"amount\":$((RANDOM%1000))}"; done \
  | podman exec -i kafka-dev $KB/kafka-console-producer.sh --bootstrap-server $BS --topic payments
for i in $(seq 1 5); do echo "{\"msg\":\"event $i\"}"; done \
  | podman exec -i kafka-dev $KB/kafka-console-producer.sh --bootstrap-server $BS --topic notifications
echo "      6 topics, ~50 messages"

# ── Consumer Groups ─────────────────────
echo "[5/6] Creating consumer groups & users..."
podman exec kafka-dev timeout 5 $KB/kafka-console-consumer.sh --bootstrap-server $BS \
...
podman exec kafka-dev $KB/kafka-configs.sh --bootstrap-server $BS \
